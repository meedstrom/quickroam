...
We "just" have to design a better regexp.  As it is, it just grabs titles and IDs, which satisfies the current purpose.  Totally replacing =org-roam-db-sync= may limit org-roam's hackability, but perhaps that's worth it.

** Thanks

Inspired by conversations on Mastodon with the [[https://github.com/rtrppl/orgrr][orgrr]] author :)
