...
They don't look up the org-roam DB at all, just rely on ripgrep.  They're fast, solving problem #1, and they always find up-to-date hits, so you can now set =org-roam-db-update-on-save= to nil to solve problem #2.

If you do the #2 fix, be aware that backlinks do not update until you manually run =M-x org-roam-db-sync RET=.
...
1. Keep parsing Org syntax, but do it faster.  Maybe rewrite =org-roam-db-update-file= to use [[https://github.com/alphapapa/org-ql][org-ql]]?

2. Don't parse Org syntax, and compromise on the kinds of data to collect.
